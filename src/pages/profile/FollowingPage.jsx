...
export default function FollowingPage() {
  ...
  useEffect(() => {
    usersApi.getFollowing(username || 'current_user').then(res => {
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-black max-w-[400px] mx-auto">
      <div className="flex items-center gap-4 px-4 py-4 border-b border-gray-200 dark:border-gray-700">
        ...
        <h2 className="font-semibold text-base text-gray-900 dark:text-gray-200">Following</h2>
      </div>
      ...
      <div>
        {following.filter(u => u.username?.toLowerCase().includes(search.toLowerCase())).map(user => (
